...
import co.edu.eam.disenosoft.universidad.vista.controlador.ControladorDialogoVerEstudiante;
...
public class DialogoVerEstudiante extends javax.swing.JDialog {
     ...
     */
    public DialogoVerEstudiante(RegistroCurso rc) {
        super();
        initComponents();
        
        controlador = new ControladorDialogoVerEstudiante();
        tfcedulaestudiante.setText(rc.getEstudiante().getCedula());
        tfnombreestudiante.setText(rc.getEstudiante().getNombre() + " " + rc.getEstudiante().getApellido());
        try {
			int creditos = controlador.sumaCreditosByAlumno(rc.getEstudiante());
			tfcreditosregistradosestudiante.setText(String.valueOf(creditos));
		} catch (Exception e) {
			e.printStackTrace();
		}
        calificacionesEstudiante(rc);
	...
	/**
     * Cargamos todas las calificaciones de un estudiante de una determinada materia
     * @param rc
     */
    public void calificacionesEstudiante(RegistroCurso rc){
    	try {
    		DefaultTableModel table = (DefaultTableModel) tablaNotas.getModel();
    		table.setRowCount(0);
    		List<Evaluacion> e = controlador.listarEvaluacionesCurso(rc.getCurso());
    		int count = e.size();
    		DecimalFormat decimales = new DecimalFormat("0.00");
			double definitiva = 0.0;
			String[] valores = new String[3];
    		List<Nota> n = rc.getNotas();
	            for(int i=0;i<count;i++){
	            	valores[0] = e.get(i).getNombre();
	            	String nota = "NC";
	        		if(!n.isEmpty()){
						for(int j=0;j<n.size();j++){
							if(n.get(j).getEvaluacion().getIdEvaluacion() == e.get(i).getIdEvaluacion()){
								definitiva += (n.get(j).getValor()*e.get(i).getPorcentaje())/100;
								nota = String.valueOf(n.get(j).getValor());
								break;
							}
						}
	    			} else{
	        		...
	        		valores[1] = String.valueOf(nota);
	        		valores[2] = String.valueOf(e.get(i).getPorcentaje());
	    			table.addRow(valores);
	            }
			lblDefinitiva.setText(String.valueOf(decimales.format(definitiva)));    		
		} catch (Exception e) {
			e.printStackTrace();
		}
    }
