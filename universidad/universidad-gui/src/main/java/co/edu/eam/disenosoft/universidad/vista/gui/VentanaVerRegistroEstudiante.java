...
import co.edu.eam.disenosoft.universidad.vista.controlador.ControladorAdministradorVerRegistroEst;
...
public class VentanaVerRegistroEstudiante extends javax.swing.JInternalFrame {
	...
	private ControladorAdministradorVerRegistroEst conVerRegistroEst;
	...
	public VentanaVerRegistroEstudiante() {
		...
		conVerRegistroEst = new ControladorAdministradorVerRegistroEst();
		setVisible(true);
    ...
    private void btnQuitarAsignaturaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnQuitarAsignaturaActionPerformed
    	...
    	try{
    		...
    		String cod = (String.valueOf(tablaHorario.getValueAt(fila, 0)));
    		String ced = tfcedulaestudiante.getText();
    		Estudiante est = conEs.buscarEstudiante(ced);
    		System.out.println(est);
    		conVerRegistroEst.elminarResgistroEsttudiante(est, cod);
	...
	public void llenarTabla(String ced){
		
		try{
			DefaultTableModel df = (DefaultTableModel) tablaHorario.getModel();
			df.setRowCount(0);

			List<RegistroCurso> cursos = conVerRegistroEst.listarRegistroEstudiante(ced);
...
//				String horario = "";
			...
			df.addRow(new Object[] { cursos.get(i).getCurso().getAsignatura().getNombre(),
					cursos.get(i).getCurso().getSesiones().get(j).getDia()
					 + " De " + cursos.get(i).getCurso().getSesiones().get(j).getHoraInicio() + " A "
					 + cursos.get(i).getCurso().getSesiones().get(j).getHoraFinal()});
				}
			}
		}catch(Exception e){
			e.printStackTrace();
